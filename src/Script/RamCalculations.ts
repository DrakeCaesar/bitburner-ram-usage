...
function parseOnlyRamCalculate(
    ...
    try {
        ...
        while (parseQueue.length > 0) {
            ...
            let script = null;
            const fn = nextModule.startsWith("./")
                ? nextModule.slice(2)
                : nextModule;
        ...
        while (unresolvedRefs.length > 0) {
            ...
            if (ref === "hacknet" && !resolvedRefs.has("hacknet")) {
                ...
                detailedCosts.push({
                    type: "ns",
                    name: "hacknet",
                    cost: RamCostConstants.HacknetNodes,
                });
            }
            ...
            if (ref.endsWith(".*")) {
                ...
                for (const ident of Object.keys(dependencyMap).filter((k) =>
                    k.startsWith(prefix)
                )) {
            ...
            try {
                ...
                const findFunc = (
                    ...
                    | { func: () => number | number; refDetail: string }
                    | undefined => {
                    if (!obj) {
                        return;
                    }
                    const elem = Object.entries(obj).find(
                        ([key]) => key === ref
                    );
                    if (
                        elem !== undefined &&
                        (typeof elem[1] === "function" ||
    ...
    walk.recursive(
        ...
        {},
        {
            WhileStatement: (
                node: Node,
                st: unknown,
                walkDeeper: walk.WalkerCallback<any>
            ) => {
                if (nodeHasTrueTest(node.test) && !hasAwait(node)) {
                    missingAwaitLine =
                        (code.slice(0, node.start).match(/\n/g) || []).length +
                        1;
                } else {
