...
function isDependencyOutOfDate(
    filename: string,
    scripts: Script[],
    scriptModuleSequenceNumber: number
): boolean {
    ...
    const depIsMoreRecent =
        depScript.moduleSequenceNumber > scriptModuleSequenceNumber;
...
function shouldCompile(script: Script, scripts: Script[]): boolean {
    ...
    if (
        script.dependencies.some((dep) =>
            isDependencyOutOfDate(
                dep.filename,
                scripts,
                script.moduleSequenceNumber
            )
        )
    ) {
        script.module = null;
...
function _getScriptUrls(
    ...
    for (const dependent of seen) {
        if (
            !script.dependents.some(
                (s) =>
                    s.server === dependent.server &&
                    s.filename == dependent.filename
            )
        ) {
            script.dependents.push({
                server: dependent.server,
    ...
    try {
        ...
        for (const node of importNodes) {
            const filename = node.filename.startsWith("./")
                ? node.filename.substring(2)
                : node.filename;

            // Find the corresponding script.
            const matchingScripts = scripts.filter((s) =>
                areImportsEquals(s.filename, filename)
            );
            ...
            transformedCode =
                transformedCode.substring(0, node.start) +
                blob +
                transformedCode.substring(node.end);
        }
        ...
        urlStack.push(
            new ScriptUrl(script.filename, blob, script.moduleSequenceNumber)
        );
        return urlStack;
    } catch (err) {
